...
void CodeGenModule::EmitGlobal(GlobalDecl GD) {
  ...
  if (LangOpts.CUDA) {
    ...
    if (const auto *VD = dyn_cast<VarDecl>(Global)) {
      ...
      // Host compilation: For __device__ functions with GTaPFunctionAttr,
      // we still need to register them in the function table for consistency
      // even though we don't emit the function body
      if (dyn_cast<FunctionDecl>(Global)) {
        if (!Global->hasAttr<CUDAHostAttr>() &&
            Global->hasAttr<CUDADeviceAttr>() &&
            Global->hasAttr<GTaPFunctionAttr>()) {
          // GTaP function body is generated by Sema side transformation
          // CodeGen side just emits the transformed AST normally
          return;
        }
      }
      if (!Global->hasAttr<CUDAHostAttr>() &&
