...
SRC="${SCRIPT_DIR}/cilksort_ideal.cu"
OUT_DIR="${SCRIPT_DIR}/out"
OUT_EXEC="${OUT_DIR}/cilksort_ideal"
...
: "${NUM_BLOCKS:=1000}"
: "${THREADS_PER_BLK:=64}"
: "${MAX_TASKS_PER_WARP:=60000}"
: "${MAX_CHILD_TASKS:=7}"
: "${TASK_KIND:=1}"

echo "============================================"
echo "  GTaP CilkSort Ideal Test Script"
echo "============================================"
...
"${CLANG_BIN}" -O3 -x cuda \
  ...
  "${SRC}" -o "${OUT_EXEC}" 2>&1 | tee "${OUT_DIR}/cilksort_ideal_compile.log"
...
"${OUT_EXEC}" /work/gc64/c64099/gpu-task-runtime/thread/util/data.bin # > "${OUT_DIR}/cilksort_ideal_output.log" 2>&1
EXIT_CODE=$?
echo "============================================"
echo ""
...
echo "  - Compile log: ${OUT_DIR}/cilksort_ideal_compile.log"
