...
SRC="${SCRIPT_DIR}/mergesort_ideal.cu"
OUT_DIR="${SCRIPT_DIR}/out"
OUT_EXEC="${OUT_DIR}/mergesort_ideal"
...
echo "============================================"
echo "  GTaP Mergesort Ideal Test Script"
echo "============================================"
...
"${CLANG_BIN}" -O3 -x cuda -fsyntax-only -v \
  -I"${GT_INC1}" -I"${GT_INC2}" \
...
"${CLANG_BIN}" -O3 -x cuda \
  ...
  "${SRC}" -o "${OUT_EXEC}" 2>&1 | tee "${OUT_DIR}/mergesort_ideal_compile.log"
...
"${OUT_EXEC}" /work/gc64/c64099/gpu-task-runtime/thread/util/data.bin # > "${OUT_DIR}/mergesort_ideal_output.log" 2>&1
EXIT_CODE=$?
echo "============================================"
echo ""
...
echo "  - Compile log: ${OUT_DIR}/mergesort_ideal_compile.log"
