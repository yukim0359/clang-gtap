...
# Device-side AST dump, filtered to declarations containing "bfs"
"${CLANG_BIN}" \
  -O0 \
  -x cuda \
  ...
  -fsyntax-only \
  -Xclang -ast-dump \
  -Xclang -ast-dump-filter=bfs \
  -I"${PROJ_DIR}/gpu-task-runtime/thread/include" \
  -I"${PROJ_DIR}/gpu-task-runtime/common" \
  -D__CUDACC__ \
  "${SRC_DIR}/bfs_ideal.cu" \
  > "${OUT_DIR}/bfs_ideal_ast_device.txt" 2>&1

echo "Filtered AST (bfs*) dumped to ${OUT_DIR}/bfs_ideal_ast_device.txt"
