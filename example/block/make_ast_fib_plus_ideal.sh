...
"${CLANG_BIN}" \
  ...
  -fsyntax-only \
  -Xclang -ast-dump \
  -Xclang -ast-dump-filter=fib_plus \
  -I"${PROJ_DIR}/runtime" \
  -D__CUDACC__ \
  "${SRC_DIR}/fib_plus_ideal.cu" \
  > "${OUT_DIR}/fib_plus_ideal_ast_device.txt" 2>&1

echo "Filtered AST (fib*) dumped to ${OUT_DIR}/fib_plus_ideal_ast_device.txt"
