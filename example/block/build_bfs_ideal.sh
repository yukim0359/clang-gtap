...
SRC="${SCRIPT_DIR}/bfs_ideal.cu"
OUT_DIR="${SCRIPT_DIR}/out"
OUT_EXEC="${OUT_DIR}/bfs_ideal"

# gpu-task-runtime includes
GT_INC1="${PROJ_DIR}/gpu-task-runtime/block/include"
GT_INC2="${PROJ_DIR}/gpu-task-runtime/common"

# launch/queue defaults (can be overridden via env)
: "${NUM_BLOCKS:=4000}"
: "${THREADS_PER_BLK:=32}"
: "${MAX_TASKS_PER_BLOCK:=50000}"
: "${MAX_CHILD_TASKS:=20400}"
: "${TASK_KIND:=1}"

echo "============================================"
echo "  GTaP BFS Ideal Test Script"
echo "============================================"
...
echo "[info]      MAX_TASKS_PER_BLOCK=${MAX_TASKS_PER_BLOCK} MAX_CHILD_TASKS=${MAX_CHILD_TASKS}"
...
"${CLANG_BIN}" -O0 -x cuda -fsyntax-only -v \
  ...
  -DMAX_TASKS_PER_BLOCK="${MAX_TASKS_PER_BLOCK}" -DMAX_CHILD_TASKS="${MAX_CHILD_TASKS}" \
...
"${CLANG_BIN}" -O3 -x cuda \
  ...
  "${SRC}" -o "${OUT_EXEC}" 2>&1 | tee "${OUT_DIR}/bfs_ideal_compile.log"
...
"${OUT_EXEC}" /work/gc64/c64099/ATOS/datasets/indochina-2004/indochina-2004_di.csr 40
EXIT_CODE=$?
echo "============================================"
echo ""
...
echo "  - Compile log: ${OUT_DIR}/bfs_ideal_compile.log"
