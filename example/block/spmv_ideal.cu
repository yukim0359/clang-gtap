...
__device__ void spmv_row(int begin, int end) {
    ...
    // Process each row in the range [begin, end)
    for (int row = begin; row < end; row++) {
        int row_idx_in_block = row - begin;
...
int main(int argc, char** argv) {
    ...
    cudaSetDevice(0);
    cudaError_t st = __gtap_init_task_runtime();
    if (st != cudaSuccess) {
    ...
    my_kernel<<<NUM_BLOCKS, THREADS_PER_BLK>>>(rows);
